      ...
      try {
        ...
        try {
          ...
          if (providerResponse && providerResponse.success && providerResponse.data) {
            ...
            setError('No active AI provider found. The API may not be configured. Please check the backend settings.');
          }
        } catch (providerError: any) {
          const errorMsg = providerError?.response?.status === 404 
            ? 'AI Provider endpoint not found. Please check if the API endpoint is correct.'
        ...
        try {
          ...
          if (modelResponse && modelResponse.success && modelResponse.data) {
            ...
            setError(prev => prev || 'No active AI model found. The API may not be configured. Please check the backend settings.');
          }
        } catch (modelError: any) {
          const errorMsg = modelError?.response?.status === 404 
            ? 'AI Model endpoint not found. Please check if the API endpoint is correct.'
